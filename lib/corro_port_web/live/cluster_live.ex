defmodule CorroPortWeb.ClusterLive do
  ...
  def handle_info({:message_change, change_type, message_map}, socket) do
    ...
    if map_size(message_map) == 0 do
      ...
      socket =
        case change_type do
          "DELETE" ->
            remove_message_from_state(socket, message_map)

          "UPDATE" ->
            update_message_in_state(socket, message_map)

          "INSERT" ->
            update_node_messages_with_new_message(socket, message_map)
  ...
  def handle_event("refresh", _params, socket) do
    ...
    socket =
      ...
      # Mark as complete after manual refresh
      |> assign(:initial_load_complete, true)
      # Clear pending rows
      |> assign(:pending_initial_rows, [])
  ...
  def handle_event("cleanup_messages", _params, socket) do
    ...
    case CorroPortWeb.ClusterLive.MessageHandler.cleanup_messages() do
      {:ok, message} ->
        # Refresh node messages after cleanup
        new_messages = CorroPortWeb.ClusterLive.DataFetcher.fetch_node_messages_data()
        Logger.warning("TODO: This will fail until I make DatFetcher take no port arg")
        Logger.debug("TODO: what is DataFetcher for anyway?")
  ...
  defp get_replication_status() do
    %{
      ...
      has_gaps: check_for_gaps(),
      conflicts: count_conflicts()
    }
  end

  defp get_total_messages() do
