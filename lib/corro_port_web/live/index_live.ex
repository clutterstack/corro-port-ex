defmodule CorroPortWeb.IndexLive do
  ...
  def handle_event("fetch_cli_members", _params, socket) do
    Logger.debug("IndexLive: 🔧 CLI cluster members fetch triggered")
  ...
  def handle_info({task_ref, {:ok, raw_output}}, socket) do
    ...
    parsed_result =
      case CorroPort.CorrosionParser.parse_cluster_members(raw_output) do
        {:ok, []} ->
          Logger.info("IndexLive: No cluster members found - single node setup")
          %{}

        {:ok, members} ->
          Logger.info("IndexLive: Parsed some CLI members")
          members

        {:error, reason} ->
          Logger.warning("IndexLive: Failed to parse CLI output: #{inspect(reason)}")
  ...
  defp get_expected_regions_from_dns do
    case DNSNodeDiscovery.get_expected_nodes() do
      {:ok, expected_nodes} ->
        ...
        Logger.debug("IndexLive: Expected regions from DNS: #{inspect(regions)}")
        regions

      {:error, reason} ->
        Logger.debug("IndexLive: Failed to get expected nodes from DNS: #{inspect(reason)}")
        []
    end
  end
  ...
  defp get_cluster_regions(updates) do
    ...
    Logger.debug("IndexLive: Message regions: #{inspect(message_regions)}")
    ...
    Logger.debug("IndexLive: Local node_id: #{local_node_id}, extracted region: #{our_region}")
    ...
    Logger.debug(
      "IndexLive: Other regions: #{inspect(other_regions)}, Our regions: #{inspect(our_regions)}"
    )
    ...
    <!-- Real-time acknowledgment count -->
          <div class="mb-4">
            <div class="flex items-center justify-between text-sm mb-2">
              ...
              <span><%= if @ack_status do %>
                {length(@ack_regions)}/{length(@expected_regions)}
                <% else %>
                0/0
                <% end %>
                regions</span>
            ...
            <div class="w-full bg-base-300 rounded-full h-2">
              <div
                class="h-2 rounded-full bg-gradient-to-r from-orange-500 to-violet-500"
