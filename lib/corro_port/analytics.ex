defmodule CorroPort.Analytics do
  ...
  def get_message_events(experiment_id, opts \\ []) do
    ...
    query = case Keyword.get(opts, :event_type) do
      nil -> query
  ...
  def get_message_timing_stats(experiment_id) do
    ...
    events
    |> Enum.group_by(&elem(&1, 0))  # Group by message_id
    ...
    case send_event do
      {_, :sent, send_time, originating_node, _} ->
        ...
        %{
          ...
          avg_latency_ms: case ack_timings do
            [] -> nil
            latencies -> latencies |> Enum.map(& &1.latency_ms) |> Enum.sum() |> div(length(latencies))
          end
        }
  ...
  def get_system_metrics(experiment_id, opts \\ []) do
    ...
    query = case Keyword.get(opts, :node_id) do
      nil -> query
