defmodule CorroPort.Test.AnalyticsFactory do
  ...
  def create_message_event(attrs \\ %{}) do
    default_attrs = %{
      message_id: "msg_#{:rand.uniform(100000)}",
      experiment_id: "test_experiment_#{:rand.uniform(1000)}",
  ...
  def create_system_metrics(attrs \\ %{}) do
    default_attrs = %{
      ...
      memory_mb: 200 + :rand.uniform(800), # 200-1000 MB
      erlang_processes: 100 + :rand.uniform(200), # 100-300 processes
  ...
  def create_message_flow(attrs \\ %{}) do
    base_attrs = %{
      message_id: "flow_msg_#{:rand.uniform(100000)}",
      experiment_id: "test_experiment_#{:rand.uniform(1000)}",
  ...
  def create_multi_node_experiment(node_count \\ 3) do
    ...
    topologies = Enum.map(1..node_count, fn node_num ->
    ...
    message_flows = for from_node <- 1..node_count,
    ...
    metrics = Enum.flat_map(1..node_count, fn node_num ->
      Enum.map(1..3, fn _ ->
  ...
  def create_performance_test_data(message_count \\ 100) do
    ...
    topology = create_topology_snapshot(%{
      experiment_id: experiment_id,
      transaction_size_bytes: 1024,
      transaction_frequency_ms: 100 # High frequency
    })
    
    # Create many message flows with varying delays
    message_flows = Enum.map(1..message_count, fn i ->
      # Introduce some artificial delay variation
      delay = :rand.uniform(50)
      
      # Create send event
      {:ok, send_event} = Analytics.record_message_event(%{
      ...
      # Create ack event
      {:ok, ack_event} = Analytics.record_message_event(%{
        message_id: "perf_msg_#{i}",
        experiment_id: experiment_id,
        originating_node: "perf_node_1", 
        target_node: "perf_node_2",
