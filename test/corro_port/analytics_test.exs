defmodule CorroPort.AnalyticsTest do
  ...
  describe "topology snapshots" do
    ...
    test "retrieves topology snapshots by experiment" do
      # Create test data
      {:ok, _} = Analytics.create_topology_snapshot(%{
        experiment_id: "exp1",
        node_id: "node1",
  ...
  describe "message events" do
    ...
    test "retrieves message events by experiment" do
      # Create test events
      {:ok, _} = Analytics.record_message_event(%{
        message_id: "msg_001",
        experiment_id: "exp1",
        originating_node: "node1",
        target_node: "node2", 
  ...
  describe "system metrics" do
    ...
    test "retrieves system metrics by experiment" do
      # Create test metrics
      {:ok, _} = Analytics.record_system_metrics(%{
        experiment_id: "exp1",
        node_id: "node1",
        cpu_percent: 25.0,
        memory_mb: 256,
  ...
  describe "experiment summary" do
    test "provides comprehensive experiment overview" do
      ...
      {:ok, _} = Analytics.record_message_event(%{
        message_id: "msg1", experiment_id: exp_id,
        originating_node: "node1", target_node: "node2", event_type: :sent
      })
      
      {:ok, _} = Analytics.record_message_event(%{
        message_id: "msg1", experiment_id: exp_id,
        originating_node: "node1", target_node: "node2", event_type: :acked  
      })
      
      # Create system metrics
      {:ok, _} = Analytics.record_system_metrics(%{
        experiment_id: exp_id, node_id: "node1",
        cpu_percent: 25.0, memory_mb: 256, erlang_processes: 100
      })
      ...
      assert summary.message_count == 2  # One send, one ack
